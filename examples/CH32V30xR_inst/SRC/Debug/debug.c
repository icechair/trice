...
// clib syscalls implementation, so the linker doesnt complain
#include <sys/stat.h>
...
__attribute__((weak)) int _lseek(int fd, long offset, int origin)
{
    //all files are empty
    return 0;
}

__attribute__((weak)) int _read(int fd, void* const buffer, unsigned const buffer_size)
{
    //nothing to read
    return 0;
}
